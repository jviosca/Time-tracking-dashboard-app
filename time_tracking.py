...
def pie_chart(df):
	...
	#plt.pie(x1, colors=colors, labels=df.index, autopct=lambda t: datetime.fromtimestamp(t/1000.0), pctdistance=0.85, explode=explode) 
	plt.pie(x1, colors=colors, labels=df.index, autopct=lambda t: pd.to_datetime(t,unit='ms').dt.strftime('%H:%M:%S:%f').str[:-7], pctdistance=0.85, explode=explode) 
